...
namespace Ymf825Dumper
{
    internal class MidiDriver
    {
        ...
        public void Start()
        {
            ...
            Driver.Section(() =>
            {
                Driver.SetGain(Gain.Level75);
                Driver.SetMasterVolume(0x28);
            });
        ...
        public void ProcessMidiEvent(Event @event)
        {
            if (@event is MidiEvent midiEvent)
                MidiInOnReceivedMidiEvent(midiEvent);
            else if (@event is SystemExclusiveEvent sysExEvent)
                MidiInOnReceivedExclusiveMessage(sysExEvent);
        }
        ...
        private void MidiInOnReceivedMidiEvent(MidiEvent midiEvent)
        {
